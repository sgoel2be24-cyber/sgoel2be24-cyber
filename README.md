...
| **[apache/airflow](https://github.com/apache/airflow)** | [#71691](https://github.com/apache/airflow/pull/71691) ✅ **merged** | `fix: resolve deprecated imports in airflow.utils.helpers` — the `__getattr__` deprecation shim called bare `__import__` on dotted paths, which returns the top-level `airflow` module instead of the target submodule, so moved functions like `render_template_as_native` raised `ImportError`. Switched to `importlib.import_module`, with regression tests. Landed as [`bb3e71d`](https://github.com/apache/airflow/commit/bb3e71da4669dc0323eda3651dac18a9eed57275). |
...
| **[risa-labs-inc/BossConsole](https://github.com/risa-labs-inc/BossConsole)** | [#324](https://github.com/risa-labs-inc/BossConsole/pull/324) ✅ **merged** | `fix(plugins): serialize concurrent first loads per class name` — made plugin class loading parallel-safe by serializing same-name definitions while allowing independent class names to load concurrently. Landed as [`56de814`](https://github.com/risa-labs-inc/BossConsole/commit/56de814122ff7f2beca67963b836ef3e2785e2f9). |
| **[risa-labs-inc/BossConsole](https://github.com/risa-labs-inc/BossConsole)** | [#325](https://github.com/risa-labs-inc/BossConsole/pull/325) ✅ **merged** | `fix(plugins): stop host resource fallback after unload` — prevented an unloaded plugin from resolving missing resources through the host while preserving the existing missing-result warning. Landed as [`895c57e`](https://github.com/risa-labs-inc/BossConsole/commit/895c57ec92c3315a48512b7b815cecb0b83516b2). |
---
